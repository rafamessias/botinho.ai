...
export default async function LandingPage() {
  ...
  const perfectFor = {
    badge: t("perfectFor.badge"),
    title: t("perfectFor.title"),
    description: t("perfectFor.description"),
    useCases: t.raw("perfectFor.useCases") as Array<{
      title: string
      description: string
    }>,
  }

  const useCaseIcons = [Utensils, Store, Briefcase]
    ...
    <main className="relative min-h-screen bg-gradient-to-b from-background via-background/60 to-muted/40">
      <header className="sticky top-0 z-50 border-b bg-background/80 backdrop-blur-md">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-4 py-2 md:px-6">
          ...
          <div className="hidden items-center gap-2 lg:flex">
            ...
            <LanguageSelector variant="compact" currentPath="/landing" />
          </div>
          <Sheet>
        ...
        <div className="flex flex-1 flex-col gap-6 text-center lg:text-left">
          ...
          <div className="flex flex-col items-center gap-3 sm:flex-row lg:justify-start">
            <Button asChild className="w-full rounded-full px-8 py-6 text-base font-semibold sm:w-auto">
              <Link href="#contact" aria-label={t("hero.primaryCtaAria")} tabIndex={0}>
                {t("hero.primaryCta")}
              </Link>
            </Button>
            <Button
            ...
            >
              <Link href="#about" aria-label={t("hero.secondaryCtaAria")} tabIndex={0}>
                {t("hero.secondaryCta")}
              </Link>
            </Button>
          </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 py-16 md:px-6">
        <div className="flex flex-col gap-6 text-center">
          <Badge className="mx-auto rounded-full bg-primary/10 px-4 py-1 text-primary">{perfectFor.badge}</Badge>
          <h2 className="text-balance text-3xl font-semibold sm:text-4xl">{perfectFor.title}</h2>
          <p className="mx-auto max-w-3xl text-pretty text-lg text-muted-foreground">
            {perfectFor.description}
          </p>
        </div>
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {perfectFor.useCases.map((useCase, index) => {
            const Icon = useCaseIcons[index]
            return (
              <Card key={useCase.title} className="rounded-2xl border border-primary/10 bg-background/80 shadow-sm">
                ...
                <CardContent className="pb-6 text-sm text-muted-foreground">{useCase.description}</CardContent>
              </Card>
            )
          })}
        </div>
