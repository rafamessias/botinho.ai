...
return`\n            <div class="yes-no-options">\n                <label class="radio-option">\n                    <input type="radio" name="q_${n.id}" value="yes" ${"yes"===t?"checked":""} onchange="window.swMinimal.setResponse('${n.id}', 'yes')">\n                    <span class="radio-option-text">${n.yesLabel||"Yes"}</span>\n                </label>\n                <label class="radio-option">\n                    <input type="radio" name="q_${n.id}" value="no" ${"no"===t?"checked":""} onchange="window.swMinimal.setResponse('${n.id}', 'no')">\n                    <span class="radio-option-text">${n.noLabel||"No"}</span>\n                </label>\n            </div>\n        `}renderSingleChoice(n){const t=this.getResponse(n.id)
return n.options.map(e=>`\n            <label class="radio-option">\n                <input type="radio" name="q_${n.id}" value="${e.id}" ${t===e.id?"checked":""} onchange="window.swMinimal.setResponse('${n.id}', '${e.id}')">\n                <span class="radio-option-text">${e.text}</span>\n            </label>\n        `).join("")}renderMultipleChoice(n){const t=this.getResponse(n.id),e=t?t.split(","):[]
return n.options.map(t=>`\n            <label class="checkbox-option">\n                <input type="checkbox" value="${t.id}" ${e.includes(t.id)?"checked":""} onchange="window.swMinimal.toggleMultiple('${n.id}', '${t.id}')">\n                <span class="checkbox-option-text">${t.text}</span>\n            </label>\n        `).join("")}renderStarRating(n){const t=this.getResponse(n.id)||0
return`\n            <div class="star-rating">\n                ${[1,2,3,4,5].map(e=>`\n                    <button type="button" class="star-rating-button ${e>t?"":"star-selected"}" onclick="window.swMinimal.setResponse('${n.id}', ${e})">\n                        <svg class="star-rating-star" width="24" height="24" viewBox="0 0 24 24" fill="${e>t?"none":"#fbbf24"}" stroke="${e>t?"currentColor":"#fbbf24"}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color: ${e>t?"#d1d5db":"#fbbf24"};">\n                            <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26 12,2"/>\n                        </svg>\n                    </button>\n                `).join("")}\n            </div>\n        `}renderLongText(n){const t=this.getResponse(n.id)||""
return`\n            <div class="long-text-input">\n                <textarea class="long-text-textarea" placeholder="" oninput="window.swMinimal.setResponse('${n.id}', this.value)">${t}</textarea>\n            </div>\n        `}renderStatement(n){return`\n            <div class="statement-content">\n                <p class="statement-text">${n.description}</p>\n            </div>\n        `}renderComplete(){this.container.innerHTML='\n            <div class="survey-widget">\n                <div class="survey-card">\n                    <div style="text-align: center; padding: 2rem;">\n                        <div style="font-size: 3rem; color: #10b981;">✓</div>\n                        <p style="margin-top: 1rem; font-size: 1.2rem;">Thank you!</p>\n                    </div>\n                </div>\n            </div>\n        ',this.isCompleted=!0,this.onComplete(this.responses),setTimeout(()=>this.onClose(),3e3)}setResponse(n,t){const e=this.responses.find(t=>t.questionId===n)
e?e.value=t:this.responses.push({questionId:n,value:t})}toggleMultiple(n,t){const e=this.responses.find(t=>t.questionId===n),i=e?(e.value||"").split(",").filter(Boolean):[]
if(i.includes(t)){const o=i.filter(n=>n!==t)
o.length?e.value=o.join(","):this.responses.splice(this.responses.findIndex(t=>t.questionId===n),1)}else i.push(t),e?e.value=i.join(","):this.responses.push({questionId:n,value:i.join(",")})}getResponse(n){return this.responses.find(t=>t.questionId===n)?.value||null}next(){const n=this.survey.questions[this.currentIndex]
!n.required||this.getResponse(n.id)?(this.currentIndex++,this.render()):alert("This question is required")}prev(){this.currentIndex--,this.render()}close(){this.onClose(),this.container.innerHTML=""}mount(n){this.container=document.getElementById(n),this.render()}}window.initSurveyWidgetMinimal=t=>(window.swMinimal=new n(t),window.swMinimal)
