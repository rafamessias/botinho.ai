...
| `/api/webhooks/whatsapp/inbound` | `WHATSAPP_WEBHOOK_SECRET` |
| `/api/cron/*` | `CRON_SECRET` (`Authorization: Bearer` or `x-cron-secret`) — invoked by Google Cloud Scheduler |
| `/api/auth/*` | NextAuth internal |

See [22-scheduled-jobs.md](22-scheduled-jobs.md) for cron setup on Firebase App Hosting.
