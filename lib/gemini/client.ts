import { FinishReason, FunctionCallingConfigMode, GoogleGenAI, Type, type Schema } from "@google/genai"
...
export const generateGeminiWithTools = async (params: {
  model: string
  ...
  if (finishReason === FinishReason.MAX_TOKENS && !functionCalls.length) {
    throw new Error(`Gemini response truncated (MAX_TOKENS). Length: ${text.length}`)
  }
