class o {
    ...
    renderCompleted() {
        ...
        this.container.innerHTML = '<div class="sw" style="min-width:300px;min-height:300px;"><div class="sc" style="--sw-bg:' + (style.backgroundColor === 'transparent' ? 'transparent' : style.backgroundColor) + ';--sw-text:' + style.textColor + ';--sw-btn-bg:' + style.buttonBackgroundColor + ';--sw-btn-text:' + style.buttonTextColor + ';--sw-font:' + style.fontFamily + ';--sw-border:' + style.border + ';--sw-radius:' + style.borderRadius + ';--sw-margin:' + style.margin + ';--sw-padding:' + style.padding + ';--sw-title:' + style.titleFontSize + ';--sw-body:' + style.bodyFontSize + ';--sw-accent:' + (style.accentColor || '#3b82f6') + ';--sw-error:' + (style.errorColor || '#ef4444') + ';--sw-border-color:' + (style.borderColor || '#d1d5db') + ';background-color:var(--sw-bg);font-family:var(--sw-font);border:var(--sw-border);border-radius:var(--sw-radius);margin:var(--sw-margin);padding:var(--sw-padding);"><div class="x" onclick="' + ref + '.handleClose()"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18"/><path d="M6 6l12 12"/></svg></div><div class="cc"><div class="ca"><div class="sc-circle"><div class="sc-check"><div class="cs"></div><div class="ck"></div></div></div></div></div><div class="sf"><div class="sb"><div class="sb-text"><p>Powered by <a href="https://opineeo.com" target="_blank" style="color:var(--sw-text);"><strong>Opineeo</strong></a></p></div></div></div></div></div>';
    ...
    renderSurvey() {
        ...
        const style = this.survey.style || this.getDefaultStyle();
        const ref = this.widgetId ? 'window.' + this.widgetId : 'window.surveyWidget';
        this.container.innerHTML = '<div class="sw" style="min-width:300px;min-height:300px;"><div class="sc" style="--sw-bg:' + (style.backgroundColor === 'transparent' ? 'transparent' : style.backgroundColor) + ';--sw-text:' + style.textColor + ';--sw-btn-bg:' + style.buttonBackgroundColor + ';--sw-btn-text:' + style.buttonTextColor + ';--sw-font:' + style.fontFamily + ';--sw-border:' + style.border + ';--sw-radius:' + style.borderRadius + ';--sw-margin:' + style.margin + ';--sw-padding:' + style.padding + ';--sw-title:' + style.titleFontSize + ';--sw-body:' + style.bodyFontSize + ';--sw-accent:' + (style.accentColor || '#3b82f6') + ';--sw-error:' + (style.errorColor || '#ef4444') + ';--sw-border-color:' + (style.borderColor || '#d1d5db') + ';--sw-outline-bg:' + (style.outlineButtonBg || 'rgba(255,255,255,.8)') + ';--sw-outline-text:' + (style.outlineButtonText || '#374151') + ';--sw-outline-hover-bg:' + (style.outlineButtonHoverBg || '#f9fafb') + ';--sw-outline-hover-border:' + (style.outlineButtonHoverBorder || '#9ca3af') + ';--sw-input-border:' + (style.inputBorder || '#d1d5db') + ';--sw-input-radius:' + (style.inputRadius || '.375rem') + ';--sw-input-bg:' + (style.inputBg || 'transparent') + ';background-color:var(--sw-bg);font-family:var(--sw-font);border:var(--sw-border);border-radius:var(--sw-radius);margin:var(--sw-margin);padding:var(--sw-padding);"><div class="x" onclick="' + ref + '.handleClose()"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6L6 18"/><path d="M6 6l12 12"/></svg></div><div class="sc-content"><div class="qtc">' + this.renderQuestion(q, style, ref) + '</div></div><div class="sf"><div class="sn"><button class="btn btn-outline btn-prev" onclick="' + ref + '.handlePrevious()" ' + (first ? 'disabled' : '') + '><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg></button><button class="btn btn-primary btn-next" onclick="' + ref + '.handleNext()" ' + (this.s ? 'disabled' : '') + '>' + (this.s ? this.getSpinnerIcon() : (last ? this.getSendIcon() : this.getNextArrowIcon())) + '</button></div><div class="sb"><div class="sb-text"><p>Powered by <a href="https://opineeo.com" target="_blank" style="color:var(--sw-text);"><strong>Opineeo</strong></a></p></div></div></div></div></div>';
        this.addStyles();
    }
    ...
    renderSingleChoiceOptions(q, resp, ref = 'window.surveyWidget') {
        ...
        if (other) {
            ...
            html += '<label class="rad"><input type="radio" name="q_' + q.id + '" value="' + other.id + '" ' + (sel ? 'checked' : '') + ' onchange="' + ref + '.handleResponseChange(\'' + q.id + '\',\'' + other.id + '\',\'' + other.id + '\',false)"><span class="rad-text">' + other.text + '</span></label>' + (sel ? '<div class="oi"><input type="text" class="oif" placeholder="Please specify..." value="' + (this.ot[q.id] || '') + '" oninput="' + ref + '.handleOtherTextChangeImmediate(\'' + q.id + '\',this.value)"></div>' : '');
        }
        return html;
    }
    ...
    renderMultipleChoiceOptions(q, resp, ref = 'window.surveyWidget') {
        ...
        if (other) {
            ...
            if (chk) {
                html += '<div class="oi"><input type="text" class="oif" placeholder="Please specify..." value="' + (this.ot[q.id] || '') + '" oninput="' + ref + '.handleOtherTextChangeImmediate(\'' + q.id + '\',this.value)"></div>';
            }
        }
        return html;
    }
    ...
    renderStarRatingOptions(q, resp, ref = 'window.surveyWidget') {
        ...
        return '<div class="stars">' + [1, 2, 3, 4, 5].map(star => '<button type="button" class="star-btn' + (star <= rating ? ' star-sel' : '') + '" onclick="' + ref + '.handleResponseChange(\'' + q.id + '\',' + star + ')"><svg class="star-svg" width="24" height="24" viewBox="0 0 24 24" fill="' + (star <= rating ? '#fbbf24' : 'none') + '" stroke="' + (star <= rating ? '#fbbf24' : 'currentColor') + '" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:' + (star <= rating ? '#fbbf24' : '#d1d5db') + ';"><polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26 12,2"/></svg></button>').join('') + '</div>';
    }
    ...
    getDefaultStyle() {
        return { backgroundColor: "transparent", textColor: "inherit", buttonBackgroundColor: "currentColor", buttonTextColor: "inherit", margin: "16px 0px", padding: "16px", border: "1px solid currentColor", borderRadius: "6px", titleFontSize: "inherit", bodyFontSize: "inherit", fontFamily: "inherit", accentColor: "currentColor", errorColor: "#ef4444", borderColor: "currentColor", outlineButtonBg: "transparent", outlineButtonText: "inherit", outlineButtonHoverBg: "rgba(0,0,0,.05)", outlineButtonHoverBorder: "currentColor", inputBorder: "rgba(0,0,0,.2)", inputRadius: ".375rem", inputBg: "inherit" };
    }
    ...
    addStyles() {
        ...
        const s = document.createElement('style');
        s.id = 'sw-styles';
        s.textContent = '@keyframes spin{to{transform:rotate(360deg)}}@keyframes slideOutRight{0%{transform:translateX(0);opacity:1}100%{transform:translateX(-50%);opacity:0}}@keyframes slideInRight{0%{transform:translateX(50%);opacity:0}100%{transform:translateX(0);opacity:1}}@keyframes slideOutLeft{0%{transform:translateX(0);opacity:1}100%{transform:translateX(50%);opacity:0}}@keyframes slideInLeft{0%{transform:translateX(-50%);opacity:0}100%{transform:translateX(0);opacity:1}}@keyframes shake{0%,100%{transform:translateX(0)}25%{transform:translateX(-2px)}75%{transform:translateX(2px)}}.sw *{box-sizing:border-box}.sw{min-width:300px;min-height:300px;font:inherit;color:inherit}.sc{box-shadow:0 4px 6px -1px rgba(0,0,0,.1),0 2px 4px -1px rgba(0,0,0,.06);max-width:300px;min-height:300px;display:flex;flex-direction:column;justify-content:space-between;overflow:hidden;position:relative}.x{position:absolute;top:.25rem;right:.25rem;width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all .2s ease;z-index:10;opacity:.7;background:transparent;border:0}.x:hover{opacity:1}.x svg{color:var(--sw-text,currentColor);transition:all .2s ease}.sc-content{padding:.5rem;margin-bottom:1rem;overflow:hidden;position:relative}.qtc{position:relative;overflow:hidden;min-height:200px;width:100%;height:auto}.qtc .qc{position:relative;width:100%;height:auto}.q-exit-right{animation:slideOutRight .2s ease-out forwards}.q-enter-right{animation:slideInRight .3s ease-out forwards}.q-exit-left{animation:slideOutLeft .2s ease-out forwards}.q-enter-left{animation:slideInLeft .3s ease-out forwards}.sf{padding:.5rem;display:flex;flex-direction:column;justify-content:space-between;padding-top:0}.sn{display:flex;width:100%;justify-content:flex-start}.btn{display:flex;align-items:center;gap:.5rem;padding:.5rem 1rem;border-radius:.375rem;cursor:pointer;transition:opacity .2s;font:inherit}.btn svg{flex-shrink:0}.spinner{animation:spin 1s linear infinite}.btn-outline{margin-right:.5rem;border:1px solid var(--sw-btn-bg,currentColor);background-color:var(--sw-btn-bg,transparent);color:var(--sw-btn-text,inherit);opacity:.7}.btn-outline:hover:not(:disabled){background-color:var(--sw-outline-hover-bg,rgba(0,0,0,.05));border-color:var(--sw-btn-bg,currentColor);opacity:1}.btn-outline svg{color:var(--sw-btn-text,inherit)}.btn-primary{border:1px solid var(--sw-btn-bg,currentColor);background-color:var(--sw-btn-bg,currentColor);color:var(--sw-btn-text,inherit)}.btn-primary:hover:not(:disabled){opacity:.9}.btn-primary svg{color:var(--sw-btn-text,inherit)}.btn:disabled{opacity:.5;cursor:not-allowed}.btn:not(:disabled):hover{opacity:.9}.sb{margin-top:1.5rem;width:100%;display:flex;justify-content:flex-start}.sb-text{display:flex;align-items:center;font-size:.75rem;color:inherit;opacity:.7}.sw-loading{display:flex;align-items:center;justify-content:center;padding:2rem}.sw-spinner{width:2rem;height:2rem;border:2px solid currentColor;border-top:2px solid transparent;border-radius:50%;opacity:.3;animation:spin 1s linear infinite}.sw-error{text-align:center;padding:2rem;color:inherit;opacity:.7}.sw-error-msg{margin:0}.qc{width:100%;height:100%;padding:0 4px}.qt{font-weight:600;margin-bottom:.5rem;color:inherit;font-size:var(--sw-title,inherit)}.qd{opacity:.8;margin-bottom:1rem;color:inherit;font-size:var(--sw-body,inherit)}.opts{margin-top:1rem;display:flex;flex-direction:column;justify-content:center;align-items:flex-start}.qr{color:var(--sw-error,#ef4444);font-size:1.2rem}.qc-error{animation:shake .5s ease-in-out}.rad,.chk{display:flex;align-items:center;gap:.5rem;margin-bottom:.5rem;cursor:pointer}.rad input[type="radio"],.chk input[type="checkbox"]{margin:0;accent-color:var(--sw-accent,currentColor)}.rad-text,.chk-text{font-size:var(--sw-body,inherit);color:inherit}.oi{margin-top:.5rem}.oif{width:100%;padding:.5rem;border:1px solid var(--sw-input-border,rgba(0,0,0,.2));border-radius:var(--sw-input-radius,.375rem);font-size:var(--sw-body,inherit);color:inherit;background-color:var(--sw-input-bg,inherit);font-family:inherit}.stars{display:flex;justify-content:center;gap:.5rem}.star-btn{padding:.25rem;background:none;border:none;cursor:pointer;border-radius:50%;transition:all .2s ease;display:flex;align-items:center;justify-content:center;outline:none}.star-btn:hover{transform:scale(1.1)}.star-btn:focus{outline:none}.star-btn.star-sel{background-color:transparent}.star-svg{width:30px;height:30px;transition:all .2s ease}.star-btn:hover .star-svg{transform:scale(1.1)}.ltxt{width:100%}.ltxt-ta{width:100%;padding:.5rem;border:1px solid var(--sw-input-border,rgba(0,0,0,.2));border-radius:var(--sw-input-radius,.375rem);font-size:var(--sw-body,inherit);color:inherit;background-color:var(--sw-input-bg,inherit);resize:none;min-height:6rem;font-family:inherit}.stmt{text-align:left}.stmt-text{font-size:var(--sw-title,inherit);font-weight:500;margin-bottom:1rem;color:inherit}.yno{width:100%}';
        document.head.appendChild(s);
    }
